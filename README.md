...
* I suggest that gauging the performance of a model can only be meaningfully accomplished in the context of the problem we are trying to solve. To me, identifying the high-risk loans is more important than identifying the healthy ones. Misclassifying a healthy loan as a high-risk loan only incurs an opportunity cost or perhaps the occasional heavy lift by customer service to explain and rectify the lapse, but letting a high-risk loan pass as a healthy loan can cause real damage to a lending institution.
